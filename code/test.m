...
for data_num = 50:55
    ...
    try
        ...
        [pks_Res_upper, locs_Res_upper] = findpeaks(filter_Res_Chest,X_Chest,"MinPeakDistance",2.0);%上のピーク検出　ここはMinPeakDistance必要ないの？
...
% legend('Filtered Signal', 'Lower Peaks');
       ...
       [filter_limit_Chest, e] = bandpass(limit_row_Chest,[0.9 5],fs_Chest,'ImpulseResponse','iir');%バンドパス0.9~5Hz
        ...
        data_name_face = fullfile(csvDir, append('A_', num, '.csv'));
        ...
        face_dis = mean(filter_face);
        disp(face_dis)
        chest_dis = mean(filter_Res_Chest);
        disp(chest_dis)
        ave_dis = face_dis - chest_dis;
        disp(ave_dis);
        ...
        A = [P_rate_Chest ave_N_PG_Chest ave_N_NG_Chest N_SG_Chest ave_PGT_Chest ave_NGT_Chest ave_A_Chest ave_Ptime_change_Chest R_rate R_depth,ave_dis];
