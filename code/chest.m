...
%保存先のエクセル
excel_name ='/Users/kajitsukasousei/Personal/Lab/HeadPosture/data/excel/Book8.xlsx';
...
for data_num = 0:10
    ...
    try
        ...
        data_name_chest = fullfile(csvDir, append('B_', num, '.csv'));
        ...
        [filter_Res_Chest, e] = lowpass(row_Chest,0.4,fs_Chest,'ImpulseResponse','iir');%バンドパス0~0.9Hz
        filter_Res_Chest_inverted = -filter_Res_Chest; %極小値をpeak値で測定するためにグラフを反転させる
        [pks_Res_lower, locs_Res_lower] = findpeaks(filter_Res_Chest_inverted,X_Chest,"MinPeakDistance",4.0);%下のピーク検出
        pks_Res_lower = -pks_Res_lower; %元に戻す
        [pks_Res_upper, locs_Res_upper] = findpeaks(filter_Res_Chest,X_Chest,"MinPeakDistance",4.0);%上のピーク検出　ここはMinPeakDistance必要ないの？
...
% 下側ピークを赤い逆三角形でプロット
plot(locs_Res_lower, pks_Res_lower, 'rv', 'MarkerFaceColor', 'b');
plot(locs_Res_upper, pks_Res_upper, 'r^', 'MarkerFaceColor', 'r');
...
% % 波形とピークをプロット
% [filter_Res_Chest2, e] = lowpass(row_Chest,0.9,fs_Chest,'ImpulseResponse','iir');%バンドパス0~0.9Hz
% figure;
% plot(X_Chest, filter_Res_Chest2);
% hold on;
% 
% % 下側ピークを赤い逆三角形でプロット
...
% xlabel('Time [s]');
% ylabel('Amplitude');
% title(['2データ番号 ', num, ' のフィルタリングされた波形と下側ピーク']);
% grid on;
% legend('Filtered Signal', 'Lower Peaks');
